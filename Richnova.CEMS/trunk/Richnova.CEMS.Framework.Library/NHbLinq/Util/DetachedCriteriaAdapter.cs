...
namespace NHibernate.Linq.Util
{
	...
	public class DetachedCriteriaAdapter : ICriteria
	{
	    ...
	    {
            return detachedCriteria.CreateAlias(associationPath, alias, joinType).Adapt(session).Add(withClause);
	    }
	    ...
	    {
            return detachedCriteria.CreateCriteria(associationPath, alias, joinType).Adapt(session).Add(withClause);
	    }
