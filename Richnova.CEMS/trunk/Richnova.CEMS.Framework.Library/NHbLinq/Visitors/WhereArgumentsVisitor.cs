...
namespace NHibernate.Linq.Visitors
{
    ...
    public class WhereArgumentsVisitor : NHibernateExpressionVisitor
    {
        ...
        private void VisitAndExpression(BinaryExpression expr)
        {
            criterionStack.Push(new List<ICriterion>());
            Visit(expr.Left);
            Visit(expr.Right);
            ...
            foreach (var crit in ands)
                CurrentCriterions.Add(crit);
        }

        private void VisitOrExpression(BinaryExpression expr)
        {
            criterionStack.Push(new List<ICriterion>());
            Visit(expr.Left);
            Visit(expr.Right);
            var ors = criterionStack.Pop();
