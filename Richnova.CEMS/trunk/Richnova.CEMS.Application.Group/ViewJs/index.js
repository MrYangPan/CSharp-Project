...
if (typeof (JSON) == 'undefined') {
    $.getScript('/Statics/Scripts/common/json2.js');
}
...
wrapper.init = function (settings) {
    ...
    $("#title").html(appSettings.globalSettings.ProductName);
    document.title = appSettings.globalSettings.ProductName;
    ...
    $('#closeMenu').menu({ onClick: wrapper.rightMenuClick });
     ...
     $('#center-tabs').tabs({
         //tools: [{ iconCls: 'icon-screen_full', handler: wrapper.setFullScreen },{ iconCls: 'panel-tool-close', handler: wrapper.tabClose }],
...
wrapper.menuAccordion = function (menus) {
    ...
    $.each(menus, function() {
        ...
        $.each(this.children, function() {
            html += utils.formatString(temple, this.url, this.iconCls, this.text);
        });
        html += '</ul>';
        $obj.accordion('add', {
            title: this.text,
    ...
    $obj.find('li').click(function() {
        ...
        var tabTitle = link.children('.nav').text();
...
wrapper.menuTree = function (menus) {
    ...
    $tree.hide().tree({
        ...
        onClick: function (node) {
            wrapper.openTab(node.text, node.url, node.iconCls);
        }
    }).tree('collapseAll').show();
};
