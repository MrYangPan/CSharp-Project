...
function formatBoolean(bool) {
    if (!bool) {
        return "<div style='width:16px;height:16px;margin:auto' class='icon-ok' />";
    } else {
        return "<div style='width:16px;height:16px;margin:auto' class='icon-cancel' />";
    }
}
function formatCheckbox(bool) {
    return formatString('<img value={0} src="/Images/{1}"/>', bool, bool ? "checkmark.gif" : "checknomark.gif");
};
function formatIcon(icon) {
    if (icon) {
        return "<div style='width:16px;height:16px;margin:auto' class='" + icon + "' />";
    }
    return "";
};
...
*/
utils.trim = function(str, c, ignoreCase) {
    if (c) {
        if ($.inArray(c, "*.?+$^[](){}|\\/") > -1) c = "\\" + c;
        var reg = new RegExp("(^" + c + "*)|(" + c + "*$)", (ignoreCase ? "gi" : "g"));
